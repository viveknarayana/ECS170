'''
Concrete MethodModule class for the CIFAR-10 CNN.
'''
    ...
    def train(self, X, y):
        ...
        for epoch in range(self.max_epoch):
            ...
            for batch_X, batch_y in train_loader:
                ...
                correct += (y_pred.max(1)[1] == batch_y).sum().item()

            epoch_loss = epoch_loss / max(total, 1)
            loss_history.append(epoch_loss)
            running_acc = correct / max(total, 1)
            print('Epoch:', epoch, 'Accuracy:', running_acc, 'Loss:', epoch_loss)
        return loss_history
    ...
    def test(self, X):
        ...
        with torch.no_grad():
            for i in range(0, X_tensor.size(0), 256):
                ...
                preds.append(self.forward(batch).max(1)[1])
