'''
Multiclass Precision / Recall / F1 using sklearn (macro, weighted, micro averages).
'''
    ...
    def evaluate(self):
        ...
        for avg in ('macro', 'weighted', 'micro'):
            results[f'precision_{avg}'] = float(
                precision_score(ty, py, average=avg, zero_division=0))
            results[f'recall_{avg}'] = float(
                recall_score(ty, py, average=avg, zero_division=0))
            results[f'f1_{avg}'] = float(
                f1_score(ty, py, average=avg, zero_division=0))
